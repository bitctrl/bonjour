...
function Service (opts) {
  ...
  this.ignore_v4 = opts.ignore_v4;
  this.ignore_v6 = opts.ignore_v6;
...
Service.prototype._records = function () {
  ...
  Object.keys(interfaces).filter( this.interface ? ( name ) => name === self.interface ? () => true ).forEach(function (name) {
    interfaces[name].forEach(function (addr) {
      if (addr.internal) return
      if (addr.family === 'IPv4' && !self.ignore_v4 ) {
        records.push(rrA(self, addr.address))
      } else if ( !self.ignore_v6 ) {
