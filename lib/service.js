...
Service.prototype._records = function () {
  ...
  Object.keys(interfaces).filter( this.interface ? ( name ) => name === this.interface ? () => true ).forEach(function (name) {
    interfaces[name].forEach(function (addr) {
      if (addr.internal) return
      if (addr.family === 'IPv4' && !this.ignore_v4 ) {
        records.push(rrA(self, addr.address))
      } else if ( !this.ignore_v6 ) {
